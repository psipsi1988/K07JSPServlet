...
public class WPServer {
	...
	public void onMessage(String message, Session session)
		...
		//동기화블록으로 공유객체에 대한 동시접근을 막는다. 
		synchronized (clients) {
			//Set컬렉션에 저장된 모든 클라이언트에게 메시지를 전송한다. 
			for (Session client : clients) {
				//단, 메시지를 보낸 '나(Me)'를 제외한 나머지에게만 전송한다. 
	...
	public void liveMessage(String message, Session session)
			throws IOException{
		System.out.println(session.getId()+":"+message);
