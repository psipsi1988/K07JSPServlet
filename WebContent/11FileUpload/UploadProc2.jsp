...
//파일업로드를 위한 MultipartRequest객체 파라미터 준비
//1. 파일을 업로드할 서버의 물리적 경로 가져오기
/*
	운영체제 별로 서버의 물리적 경로는 달라질 수 있으므로 파일이 
	업로드되는 정확한 위치를 알기 위해서 반드시 필요한 정보다. 
*/
String saveDirectory = application.getRealPath("/Upload");

//2. 업로드할 파일의 최대 용량 설정 (바이트 단위)
/*
	만약 파일을 여러 개 업로드 한다는 각각의 용량을 합친 용량이 최대용량이 된다. 
	Ex) 500Kb * 2 = 1000Kb
*/
...
try{
	...
	*/
	...
	//파일명을 변경하기 위해 현재 시간을 가져온다. 
	/*
		아래 서식문자 중 대문자 S는 초를 1/000단위로 가져옴. (0~999)
	*/
	...
	//파일의 확장자를 가져옴
	/*
		파일명에 닷(.)이 2개 이상 포함될 수 있으므로 lastIndexOf()로 
	...
	*/
	...
	※파일객체.separator : 파일 경로를 나타낼 때 윈도우는 역슬러시(\)
		리눅스는 슬러시(/)를 사용하는데 OS에 따라 구분 기호를 자동으로 
		변환하여 설정해주는 역할을 한다. 
	*/
	...
	*/
	...
	/*
		파일업로드시 예외가 발생하면 request영역에 오류메시지를 저장하고 
		업로드폼 페이지로 포워드한다. 
	*/
...
<body>
	...
	<ul>
		...
		<li>서버에 저장된 파일명(변경 전) : <%=mr.getFilesystemName("chumFile1") %></li>
		<li>서버에 저장된 파일명(변경 후) : <%=realFileName%></li>
		
		<li>콘텐츠타입 : 
		<%=mr.getContentType("chumFile1") %></li>
		<li>파일크기 : 
			<%=(int)Math.ceil(newFile.length()/ 1024.0) %>Kb
		</li>
		<li>이미지표현:
