...
- 애플리케이션 영역에 저장 속성은 모든 JSP(Servlet) 페이지에서 공유된다. 
- 링크를 통해 새로운 페이지에 대한 요청이나, 포워드, 인클루드 등 모든  페이지에서 
	영역이 공유된다. 이런 페이지들은 하나의 웹애플리케이션 내에  존재하기 때문이다. 
...
//애플리케이션 영역에 리스트와 맵 저장
application.setAttribute("lists", lists);
application.setAttribute("maps", maps);

//페이지 영역의 리퀘스트영역에 각각 속성 저장
...
<body>
	...
	<!-- 
		새로운 페이지에 대한 요청이므로 page영역, request영역 둘 다 공유되지 않는다. 
	...
	<!-- 
		외부파일 인클루드 : include 지시어를 통해 페이지를 삽입하면 
			파일의 내용이 그대로 포함된 후 컴파일되므로 하나의 페이지라 간주하면 된다. 
