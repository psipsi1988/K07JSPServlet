...
<body>
	...
	<%--
	최초 요청된 페이지와 포워드로 전달된 페이지는 request영역을 공유하게 된다. 
	URL주소창에는 최초 요청된 페이지의 경로명이 보여지지만 출력되는 페이지는 
	새롭게 전달된 페이지의 내용을 출력하게 된다. 
	그러므로 사용자는 페이지 이동을 인지하지 못하지만 내부적으로 페이지 전달(포워드)된 것이다. 
	...
	<%--
	sendRedirech()는 JS의 location.href와 동일한 기능으로 새로운 
	페이지에 대한 요청을 하게 된다. 이 경우 request영역은 공유되지 않는다. 
	웹브라우저의 URL주소창에는 새롭게 요청된 페이지의 경로명이 보이게 된다. 
	 --%> 
