...
<%
/*
모든 영역(페이지, 리퀘스트, 세션, 애플리케이션)에는 모든 타입의 객체를 저장할 수 있다. 
Object타입으로 자동 형변환되어 저장되고 사용시에는 각 객체로 형변환 후 사용하면 된다. 
향후 EL(표현언어)을 사용하면 형변환 없이 사용할 수 있다. 


1. page영역
	-페이지 영역에 저장된 속성은 해당 페이지에서만 공유되고 페이지를 
	벗어나는 순간 소멸된다. 
	-페이지 영역에는 pageContext 내장객체를 사용하여 영역을 사용한다. 
*/
...
<body>
	...
	<%
	/*
	page영역에 저장될 때 Object타입으로 자동형변환되어 저장되므로 
	읽어올 때도 Object타입으로 가져오는 것이기본이다. 
	*/
	Object obj = pageContext.getAttribute("pageDate");
	String dateString = "";
	//Date타입으로 형변환 가능한지 확인 후...
	...
	String m1Str = String.format("아이디:%s, 비번:%s, 가입일:%s", 
				m1.getId(), 
				m1.getPass(), 
				m1.getName(), 
				m1.getRegidate());
	...
	<%--
		페이지 영역에 저장된 속성값은 새로운 페이지에 대한 요청이 발생하면 
	...
	<br/>
	<%--
		외부파일을 인클루드 하는 경우 외부문서는 원본 그대로 현재 문서어
		포함된 후 컴파일되므로  incude 지시어를 사용하는 경우 
