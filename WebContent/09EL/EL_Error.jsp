...
<body>
	<fieldset>
		...
		<!-- 
			EL은 null에 대해 어느 정도까지는 처리를 해주므로 아래는 
			에러가 발생하지 않는다. 
		 -->
		...
		<h2>인덱스로 접근시(배열이나 리스트 계열 컬렉션)</h2>
		<%
			...
			for(String mountain  : mountains)
				list.add(mountain);
		%>
		<!-- JSTL의 set태그를 통해 변수로 선언. 이후 EL에서 사용 가능함. -->
		...
		<!-- JSTL의 forEach 태그를 통해 반복출력. 해당 문장은 
		배열이나 컬렉션의 크기만큼 반복하는 확장for문의 형태로 기술됨.  -->
		<h4>배열</h4>
		...
		<ul>
			<li>존재하지 않는 속성 : \${eldto.addr }
				:
				\${eldto.addr } [PropertyNotFoundException 발생]
				</li>
				
			<!-- DTO객체에서 getId()를 주석처리 후 테스트해볼 것 -->
			<li>속성은 존재, Getter가 없는 경우 : ${eldto.id }
			 	:
		 		\${eldto.id } [500에러 발생]
		 		</li>
		 	<li>el변수명이 틀린 경우 : \${noeldto.pwd }
		 		:
		...
		<ul>
			<li>\${requestScope.nop } : ${requestScope.nop }
					...
					개발자 마음대로 정할 수 있기 때문]</li>
					
			<!-- EL을 통해 네 가지 영역에 접근할 때는 XXXScope 패턴으로 사용한다.  
			pageContext는 JSP에서 접근시 사용한다. -->
			<li>\${pageContext.nop } : \${pageContext.nop }
					[컴파일오류 발생 -> 내장객체명은 정해져있기 때문]</li>
		</ul>
		...
		<h2>EL 변수에 값 할당시 [톰켓8.0부터는 정상. 이전 버전은 에러]</h2>
