...
<body>
	...
	*/
	...
	<!-- 
		최초 페이지를 실행했을 때에는 파라미터가 없으므로 0으로 간주되어 계산됨
		만약  
		해당 페이지?myNumber=20인 경우 결과 30 출력됨
		해당 페이지?myNumber= 인 경우 0으로 간주되어 10 출력됨
		해당 페이지?myNumber=삼인 경우 문자는 숫자로 변경 불가하므로 에러 발생됨. 
	 -->
	...
	값은 출력되지 않고 null로 인식하게 된다. 
	JSP의 변수를 EL에서 사용할 수 없는 이유는 EL은 네 가지 영역에 저장된
	속성들만 사용하기 때문이다. JSTL도 동일한 특성을 가지고 있다. 
	*/
	String varScriptLet = "스크립트렛 안에서 변수 선언";
	%>
	<!--  null값으로 인식되므로 0으로 간주된다. 결과는 100출력 -->
	
	\${varScriptLet+100 } : ${varScriptLet+100 }<br/>
	
	<!-- 
	JSP코드에서 선언한 변수를 EL에서 사용해야 할 경우에는 JSTL의 set 태그를 
	사용해서 변수를 선언한다. JSP에서 즉시 선언하려면 영역에 저장한다. 
	 -->
	
	<c:set var="elVar" value="<%=varScriptLet %>" />
	\${elVar } : ${elVar } <!-- 99로 재할당되어 99 출력됨 -->
	
	
	<!-- 
		Tomcat8.0부터는 EL에서 변수할당이 가능해졌다. 
		하지만 개발시에는 실제 서비스할 웹서버의 버전을 확인후 사용 여부를 결정한다. 
		EL은 전통적으로 값을 표현(출력)하는 용도로 사용됐으므로 
		기표현용으로만 사용하는 것이 좋다. 
	 -->
	...
	<!-- 
		EL에서는 정수와 정수를 연산하더라도 '실수'의 결과가 나올 수 있다. 
		즉, 자동형변환되어 출력된다. 
	...
	String클래스의 co와mpareTO()메소드와 같은 방식으로 비교한다. 
	...
	단, 실제 숫자 비교시에는 일반적인 숫자비교가 이루어진다. 
	 ...
	 <ul>
	 	...
	 	<!-- 
	 		Java에서는 문자열을 비교할 때 equals()메소드를 통해 비교하지만 
	 		EL에서는  == 형태로 비교한다. 
	 	 -->
	...
	<h3>EL의 삼항연산자</h3>
	\${10 gt 9 ? "참이다" : "거짓이다" } ${10 gt 9 ? "참이다" : "거짓이다" }
	...
	<ul>
		...
		<li>${empty ellengthZero ? "배열 크기가 0임" : "배열크기가 0이 아님" }</li> 
