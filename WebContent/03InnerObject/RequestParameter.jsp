...
<body>
 ...
 /*
 전송방식 get/post 상관없이 getParameter("파라미터명") 메소드로 
 폼값을 받을 수 있다. 만약 값이 입력되지 않았다면 길이가 0인 String객체를 반환하고
 파라미터명이 틀린 경우에는  null를 반환한다. 
 */
 ...
 if ( !(name==null || name.equals("")) ) {
	 ...
	 /*
	스크립트렛 안에서 JS를 선언할 때는 out.println()을 통해서 
	아래와 같이 사용할 수 있다. 
	 */
	 out.println("<script>");
...
/*
radio는 폼값을 아래 두 가지 방법을 모두 사용할 수 있다. 
항목은 여러 개이나 실제 선택 가능한 항목은 하나이기 때문이다. 
*/
 
 //방법1 : text 타입과 동일한 형태로 폼값을 받을 때 ... 
 //String sexStr = request.getParameter("sex");
 //방법2 : checkbox처럼 배열형태로 폼값을 받을 때...
...
*/
 ...
 if(favo != null){
	 for(int i=0; i<favo.length ; i++){
		 ...
		 문자열의 마지막 부분에 ,를 제거하기 위해 if문을 통해
		 체크하고 있다. 
		 */
		 if(i==favo.length-1){
			 favStr += favo[i];
 ...
 /*textrea를 통해 입력받은 값은 엔터키(줄바꿈기호\r\n)가 포함되어 있다. 
 웹브라우저에 출력할 때는 특수기호를 <br/>태그로 변환 후 출력해야 한다. */
 String self_intro = 
...
/*
getParameterNames() : 폼값의 이름을 한꺼번에 받을 때 사용하는 
	request 내장객체의 메소드로 반환타입은 Enumeration형이다. 
	
	hasMoreElements() : 객체에 남은 폼값이 있는지 검사
	nextElement() : 받은 폼값 중 다음 폼의 이름을 반환
*/
