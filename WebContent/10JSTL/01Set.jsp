...
<body>
	...
	set 태그 속성
		...
		scope : 영역을 지정. 문자열만 가능하고 네 가지 영역을 지정할 수 있음. 
				기본값은 가장 좁은 page영역. 
	...
	<h3>EL로 출력</h3>
	<!-- 
		EL로 영역의 속성을 읽어올 때는 XXXScope를 생략하면 가장 좁은 page영역을 읽어오게 된다.
		즉, 아래는 모두 page영역을 읽어오게 된다.  
	 -->
	<ul>
	...
	<c:set var="argsMember"
		...
		<!-- 영역에 저장될 때는 Object타입으로 자동형변환되므로 사용시
		...
		property : 값을 지정한속성. 표현식, EL, 값 모두 표현 가능
			-자바빈인 경우 : 속성명(멤버변수)
			-Map 컬렉션인 경우 : 키값
		...
		<!-- 
		Map계열의 컬렉션은 Key값을 통해 value에 접근한다. 
